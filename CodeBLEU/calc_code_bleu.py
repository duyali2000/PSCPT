...
# preprocess inputs
pre_references = [[remove_comments_and_docstrings(x.strip(), lang) for x in open(file, 'r', encoding='utf-8').readlines()] \
                for file in args.refs]
hypothesis = [remove_comments_and_docstrings(x.strip(), lang) for x in open(args.hyp, 'r', encoding='utf-8').readlines()]
...
from preprocess import code_replace

# calculate ngram match (BLEU)

#re.split('。|！|？',text)
tokenized_hyps = [code_replace(re.split('|\'|}|{| |.|,|]|;|\+|-', x)) for x in hypothesis]
tokenized_refs = [[code_replace(re.split('|\'|}|{| |.|,|]|;|\+|-', x)) for x in reference] for reference in references]

# tmp_hyps = []
# tmp_ref = []
# for hyp, ref in zip(tokenized_hyps, tokenized_refs):
#     if(len(hyp) > len(ref[0]) + 3):
#         print(len(hyp), len(ref[0]))
#         continue
#     print(hyp, ref[0])
#     tmp_hyps.append(hyp)
#     tmp_ref.append(ref)
# tokenized_hyps = tmp_hyps
# tokenized_refs = tmp_ref
